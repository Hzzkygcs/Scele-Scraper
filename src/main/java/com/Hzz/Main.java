...
import static com.Hzz.Utility.Constants.DELETED_URL_SCRAP_MSG;
...
import com.Hzz.Model.UrlEntryPoint;
import com.Hzz.Utility.*;
import com.Hzz.GUI.GuiCheckbox;
import com.Hzz.GUI.GuiConsole;
    ...
    private static void login_scele(){
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null,
                                          "Couldn't login to scele.cs.ui.ac.id\nPlease recheck the username and password you've given",
...
//                out.println(Repr.repr(temp));
        ...
        public Collection<String> processNextUrlToBeScrapped(HtmlPage result) {
            ...
            for (DomNode obj: elements_with_href){
                ...
                if (BlacklistUrl.create().contains(url))
                    continue;
    ...
    private static ArrayList<ScrapData> getDifference(){
        ...
        Set<String> blacklisted_urls = BlacklistUrl.create().__get_set();
        Set<String> keys_previous_data = Sets.difference(previous_data.keySet(), blacklisted_urls);
        Set<String> keys_current_data = Sets.difference(current_data.keySet(), blacklisted_urls);
        ...
        for (String key: deleted_URLs){
            ...
            ScrapData difference = new ScrapData(prev, curr, DELETED_URL_SCRAP_MSG);
            differences.add(difference);
        }
